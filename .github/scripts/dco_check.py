...
def process_commits(
    ...
    for commit in commits:
        ...
        if not any(
            sign_off_matches_author(name, email, commit.author_name, commit.author_email) 
            for name, email in sign_offs_name_email
        ):
            infractions[commit.hash].append(
...
# This function checks if the sign-off matches the author. For example, the expected sign-off is "John Doe <john.doe@example.com>", and the author is "John Doe" and the email is "john.doe@example.com".
# It will return true in the case where the author email is a GitHub login. For example, if the author email is "1234567890+john.doe@users.noreply.github.com", the sign-off name will be "john.doe".
def sign_off_matches_author(
    sign_off_name: str,
    sign_off_email: str,
    author_name: Optional[str],
    author_email: Optional[str],
) -> bool:
    if (sign_off_name, sign_off_email) == (author_name, author_email):
        return True

    if author_email:
        match = re.match(r'^\d+\+([^@]+)@users\.noreply\.github\.com$', author_email)
        if match:
            github_login = match.group(1)
            return sign_off_name == github_login
